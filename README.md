...
5. **Java コードを書く**
    ...
    1. 解説(9行): `@Command`アノテーションで、このコマンド全体のヘルプを指定
    2. 解説(13行): `@Option` アノテーションで、`-n 文字列` の形の入力引数を`name`属性に格納するように指定
    3. 解説(16行): `Callable`インターフェースのメソッド `call()`を実装する。`execute()`を呼ぶとその中で`call()`が呼ばれるので、実質`execute()`を実装しているのと同じ意味になる。
    4. 解説(23行): 
